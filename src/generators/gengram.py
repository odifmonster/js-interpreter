...
def get_delta_ops(groups: list[str]) -> tuple[Delta, Ops, Ops]:
    delta: Delta = {}
    binops: Ops = set()
    unops: Ops = set()
...
def add_operator(dest: str, binops: Ops, unops: Ops) -> None:
    outfile = open(dest, "a")
...
def main():
    ...
    delta, binops, unops = get_delta_ops(groups)
    add_operator(PARSE_TYPES, binops, unops)
